...
export class SupabaseConnector
  ...
  async register(email: string, password: string, fullName: string, phone?: string) {
    ...
    // No session yet if email confirmation is required (enabled by default outside
    // local dev) — updateSession(null) is a no-op in that case, which is correct.
    this.updateSession(data.session);
  }

  /** Restores a persisted session (if any) on app start, without forcing a new login. */
