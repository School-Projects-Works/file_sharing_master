...
/**
 * Connects PowerSync to the backend. Call this once there is a real Supabase
 * session (a restored session on app start, or right after a successful login) —
 * unlike the demo template, we don't auto-sign-in anonymously, so there is nothing
 * to connect with until the user actually logs in.
 */
